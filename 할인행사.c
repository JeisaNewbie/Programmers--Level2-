...
int arr_of_data[100001] = {0,};
int front = 0;
int rear = 0;

void init_arr (const char* want[], size_t want_len, const char* discount[], size_t discount_len, int * cp_discount) //discount에 저장된 문자열을 숫자로 변환
{
...
int solution(const char* want[], size_t want_len, int number[], size_t number_len, const char* discount[], size_t discount_len) {
    int *cp_discount = calloc (discount_len, sizeof(int));
    int *cp_want = calloc (want_len + 1, sizeof(int));
    int *cp_num = calloc (want_len + 1, sizeof(int));
    int answer = 0;
    ...
    for (size_t i = 0; i < discount_len; i++)
    {
        ...
        {
            memset (cp_num, 0, sizeof (int) * (number_len + 1));
            rear = front;
